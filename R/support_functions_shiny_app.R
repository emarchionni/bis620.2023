...
#' @importFrom dplyr rename arrange distinct everything
...
#' @importFrom dplyr collect arrange summarize group_by left_join n desc
#' @importFrom stats reorder
#' @importFrom utils head
#' @importFrom ggplot2 geom_col xlab theme_bw labs theme element_text
#' @noRd
...
# Note PROBLEM: we don't need any function to be defined here. We just
...
#' @importFrom dplyr filter collect summarise distinct group_by ungroup mutate right_join
#' @importFrom utils head
...
plot_adverse_events <- function(studies, reported_events){
  ...
  x <- x |>
    mutate(event_type = replace(event_type, "other", "nonserious"))
...
#' @importFrom dplyr collect left_join summarize filter n summarise group_by
#' @importFrom ggplot2 geom_map labs theme element_text
#' @noRd
...
#'  will be shown in the title of the plot
#'  Also the plot will show the 95% confidence planes (ellipses) for observations of each sponsor type. However,
#'  if the number of observations for some sponsor type is small, then the warning will appear and no ellipse
#'  will be show for the observations which have this sponsor type.
#' @param studies_filt table/tibble/data.frame, input `studies` table filtered by the query
#' nxd where n is the number of filtered studies and d is the number of features
...
#' @importFrom dplyr mutate collect across left_join summarize n filter
#' @importFrom tidyr replace_na
#' @importFrom tidyselect where
#' @importFrom ggplot2 geom_point labs theme element_text
#' @noRd
...
plot_detailedness <- function(studies_filt, calculated_values){
  ...
  ggplot(detailedness_data, aes(x = total_outcomes, y = 0.1 + log(enrollment, 10), col = source_class)) +
    geom_point() +
    # adding ellipses to calculate 95% confidence plane for better graph readability
    # note that if the number of observations is small, the confidence ellipse may be not shown
    # and because of that the console may show warnings about missing values and non-finite values
    # stat_ellipse() +
